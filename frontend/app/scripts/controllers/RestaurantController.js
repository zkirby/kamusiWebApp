...
angular.module('webappApp')
    .controller('RestaurantController', ['$scope', '$uibModal', 'lookupService', 'elementService', 'nameAddrService', 'submitService',  function ($scope, $uibModal, lookupService, elementService, nameAddrService,submitService) {
        ...
        $scope.model = {
		 ...
		 restaurant_langauge: nameAddrService.get('lang')
        };
        
        //will have to be retrieved
        var bestSections = [
{name: "Appetizers", 
	subsectionButtons: ["Appetizers"]}, 
{name: "Childrens Menu", 
	subsectionButtons: ["Plates", "Bowls"]},
{name: "Main dishes", 
	subsectionButtons: ["Pasta", "Pizza", "Sandwiches"]},
{name: "Salads", 
	subsectionButtons: ["Vegetarian", "Meat"]},
{name: "Side Dishes", 
	subsectionButtons: ["Sides"]},
{name: "Drinks",
	subsectionButtons: ["Wine", "Beer", "Soda", "Tea"]},
{name: "Deserts",
	subsectionButtons: ["Cakes", "Pies"]}];
...
//Provides ingredients from the ingredients text area as opposed to the description
	   $scope.ingredientsFromTextArea = function()
	   {
		console.debug($scope.model.menuElem.item.ingredientArea);
		console.log("test");
            $scope.model.potentialIngredients = [];
            $scope.model.menuElem.item.ingredients = [];
            lookupService.lookup($scope.model.menuElem.item.ingredientArea).then(
                function(resp){
                    resp.data.forEach(function(el) {
                        $scope.model.potentialIngredients.push({name: el});
                    });
